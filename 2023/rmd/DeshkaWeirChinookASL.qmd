---
title: "Deshaka Weir Chinook ASL"
date: 9/28/2023
format:
  html:
    toc: true
...
# There is an issue with how Quarto renders the .qmd and where it is defining the directory. I currently do not know the proper solution but adding in the here() function seems to allow it to run properly. Ideally, the .qmd file lives in the rmd/qmd folder in DeshkaWeir > 2023 > rmd/qmd and the document would use the project directory that is in DeshkaWeir / DeshkaWeir.Rproj.  The function above "here()" seemingly makes that happen, but when you use getwd() within this document it states the working directory is H:/R/DeshkaWeir/2023/rmd, which it should not be and should ideally remain relative to the R project. 
```
...
strat2 <- interval(ymd("2023-06-20 UTC"),ymd("2023-06-22 UTC"))
...
The KS test suggests that the sampling process was not proportionally representative of the entire run. The operational plan specifies that strata should be assigned such that each stratum to contains approximately a quarter of the total run. Consequently, the first stratum should extend from 6/1/23 to 6/18/23. By including 6/19 in the first stratum, we are able to better align the sampling proportions with the passage proportions and simultaneously allow sampling and passage to contain roughly 25% of the total run / samples.
